...
namespace Les5Oefenbundel1
{
    class Program
    {
        ...
        {
            List<Player> players = new List<Player>(GetPlayers());

            foreach (var player in players.Where(x => x.DateOfBirth.Year > 1969).ToList())
            {
            ...
            foreach (var player in players.Where(x => x.Fines.Count > 0).ToList())
            {
                Console.WriteLine($"{player.Id} {player.LastName} {player.DateOfBirth} {player.Fines.Max(x => x.Amount)}");
            }
            ...
            {
                Console.WriteLine($"{player.Id} {player.LastName} {player.Fines.Sum(x => x.Amount)}");
            }

            Console.WriteLine();

            foreach (var player in players.OrderBy(x => x.Fines.Sum(x => x.Amount)))
            {
                Console.WriteLine($"{player.Id} {player.LastName} {player.Fines.Sum(x => x.Amount)}");
            }
            ...
            {
                var player = players.OrderBy(x => x.DateOfBirth).First();

                Console.WriteLine($"{player.LastName} {player.DateOfBirth}");
            }

            Console.WriteLine();

            foreach (var player in players.Where(x => x.Fines.Count > 0 && x.DateOfBirth.Year > 1955))
            {
                Console.WriteLine($"{player.Id} {player.LastName} {player.DateOfBirth} {player.Fines.Count}");
            ...
            foreach (var playerGroup in players.GroupBy(x => x.DateOfBirth.Year).ToList())
            {
                Console.WriteLine($"{playerGroup.Key} {playerGroup.Count()}");

                foreach (var player in playerGroup)
                {
                    Console.WriteLine($"\t {player.Id} {player.LastName}");
                }
            }
        }
        ...
        private static List<Player> GetPlayers()
        {
            List<Player> players = new List<Player>
            {
                ...
                new Player{Id=6,LastName="Permentier",Initials="R",DateOfBirth=new DateTime(1964,6,25),Gender='M',YearOfJoining=1977,Street="Hazensteinln",Number="80",ZipCode="1234KK",City="Den Haag",Phone="070-476537",FederationNr="8467",Fines=new List<Fine>{new Fine { Id=1,PlayerId=6,DatePayed=new DateTime(1980,12,08),Amount=100M}}},
                ...
                new Player{Id=8,LastName="Niewenburg",Initials="B",DateOfBirth=new DateTime(1962,7,8),Gender='V',YearOfJoining=1980,Street="Spoorlaan",Number="4",ZipCode="6584WO",City="Rijswijk",Phone="070-458458",FederationNr="2983",Fines=new List<Fine>{new Fine { Id=6,PlayerId=8,DatePayed=new DateTime(1980,12,8),Amount=25M}}},
                new Player{Id=27,LastName="Cools",Initials="DD",DateOfBirth=new DateTime(1964,12,28),Gender='V',YearOfJoining=1983,Street="Liespad",Number="804",ZipCode="8457DK",City="Zoetermeer",Phone="079-234857",FederationNr="",Fines=new List<Fine>{new Fine { Id= 3,PlayerId=27,DatePayed=new DateTime(1983,9,10),Amount=100M},new Fine { Id=8,PlayerId=27,DatePayed=new DateTime(1984,11,12),Amount=75M}}},
                ...
                new Player{Id=44,LastName="Bakker, de",Initials="E",DateOfBirth=new DateTime(1963,1,9),Gender='M',YearOfJoining=1980,Street="Lawaaistraat",Number="23",ZipCode="4444LJ",City="Rijswijk",Phone="070-368753",FederationNr="1124",Fines=new List<Fine>{new Fine { Id=2,PlayerId=44,DatePayed=new DateTime(1981,5,5),Amount=75M},new Fine { Id=5,PlayerId=44,DatePayed=new DateTime(1980,12,8),Amount=25M},new Fine { Id=7,PlayerId=44,DatePayed=new DateTime(1982,12,30),Amount=30M}}},
                ...
                new Player{Id=104,LastName="Moerman",Initials="D",DateOfBirth=new DateTime(1970,5,10),Gender='V',YearOfJoining=1984,Street="Stoutlaan",Number="65",ZipCode="9437AO",City="Zoetermeer",Phone="079-987571",FederationNr="7060",Fines=new List<Fine>{new Fine { Id=4,PlayerId=104,DatePayed=new DateTime(1984,12,8),Amount=50M}}},
